...
bib.df = lapply(bib, function(bibi){
  data.frame(title=bibi$title %>% gsub('\\{','',.) %>% gsub('\\}','',.),
             ...
             journal=ifelse(!is.null(bibi$journal),
                            gsub('.*\\}\\{(.*)\\}', '\\1', bibi$journal),
             ...
             press=ifelse(is.null(bibi$pressurl), '', bibi$pressurl),
...
htmlCit <- function(df){
  ...
  cat('<div class="col-md-10"><span itemprop="name">', df$title, '. <a target=\"_blank\" href="https://doi.org/',
      df$doi, '">', df$journal, ' ', df$year, '.</a></span><div class="authorpub">', df$author, '</div>', preprintdoi, scripts, press, '</div>', sep='')
  ## Badges
